...
import { Shield, Zap, Layout, Bot, Crown, Sparkles } from 'lucide-react';
...
const FeatureCard = ({ 
  ...
  <motion.div
    ...
    viewport={{ once: true }}
    className="bg-[#141417] p-10 rounded-[3rem] h-full border border-[#1A1A1E] hover:border-[#5865F2]/30 transition-colors"
  >
    <div className="w-14 h-14 rounded-2xl bg-[#5865F2]/10 flex items-center justify-center text-[#5865F2] mb-8">
      <Icon size={28} />
    </div>
    <h3 className="text-2xl font-bold text-white mb-4 tracking-tight">{title}</h3>
    <p className="text-white/30 text-base leading-relaxed mb-4">{desc}</p>
    <div className="flex items-center mt-auto pt-4">
      <Sparkles className="text-[#FFD700]" size={16} />
      <span className="text-[#FFD700] text-sm font-bold ml-2">{benefit}</span>
    </div>
  </motion.div>
);
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <FeatureCard 
            ...
            benefit="+75% less moderation time"
          />
          <FeatureCard 
            index={1} 
            icon={Layout} 
