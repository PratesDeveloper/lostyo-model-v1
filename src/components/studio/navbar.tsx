  ...
  useEffect(() => {
    const checkLogin = async () => {
      ...
      if (loggedCookie === 'true' && robloxId) {
        setIsLogged(true);
        // Usando Server Action para evitar 401
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center py-8 px-6">
      <div className="w-full max-w-5xl h-14 glass rounded-full flex items-center justify-between px-6 border-white/5">
        ...
        <div className="flex items-center gap-4">
          {isLogged && profile ? (
            <>
              {/* Link do Dashboard visível no Header */}
              <Link href={isDeveloper ? "/dashboard-admin" : "/"}>
                <button className="px-5 py-2 bg-white/5 hover:bg-white/10 text-white rounded-full text-[10px] font-black uppercase tracking-widest transition-all hidden xs:block">
                  Dashboard
                </button>
              </Link>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger className="outline-none">
                  <div className="flex items-center gap-2 pl-1 pr-3 py-1 bg-white/5 hover:bg-white/10 rounded-full transition-all border border-white/5 cursor-pointer">
                    ...
                    <span className="text-[10px] font-black text-white/70 uppercase tracking-widest hidden sm:block">
                      Account
                    </span>
                  </div>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56 bg-[#0D0D0F] border-white/10 text-white rounded-2xl p-2 mt-4 shadow-2xl">
                  <div className="px-3 py-4 border-b border-white/5 mb-2">
                    <div className="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-1">Authenticated</div>
